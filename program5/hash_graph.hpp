...
bool HashGraph<T>::empty() const {
        return node_values.size() == 0 && edge_values.size() == 0;
}
...
T HashGraph<T>::edge_value(NodeName origin, NodeName destination) const {
        if (!edge_values.has_key(Edge(origin, destination)))
            throw GraphError("Edge not in graph");
...
int HashGraph<T>::in_degree(NodeName node_name) const {
        if (!node_values.has_key(node_name))
            throw GraphError("Node not in graph");
...
int HashGraph<T>::degree(NodeName node_name) const {
        ...
        return node_values[node_name].out_nodes.size() +
                node_values[node_name].in_nodes.size();
}
...
void HashGraph<T>::add_edge (NodeName origin, NodeName destination, T value) {
    ...
    node_values[destination].from_graph = this;
}
...
void HashGraph<T>::remove_node (NodeName node_name){
        if (!has_node(node_name))
            return;
        NodeMap copy_node_values(node_values);
        EdgeMap copy_edge_values(edge_values);
        node_values.erase(node_name);
        for (auto node: copy_node_values) {
            for (auto edge: node.second.out_edges)
                if (edge.first == node_name || edge.second == node_name)
                    node_values[node.first].out_edges.erase(edge);
            for (auto edge: node.second.in_edges)
                if (edge.first == node_name || edge.second == node_name)
                    node_values[node.first].in_edges.erase(edge);
            for (auto in: node.second.in_nodes)
                if (in == node_name)
                    node_values[node.first].in_nodes.erase(in);
            for (auto in: node.second.out_nodes)
                if (in == node_name)
                    node_values[node.first].out_nodes.erase(in);
        }
        for (auto pair: copy_edge_values)
            if (pair.first.first == node_name || pair.first.second == node_name)
                edge_values.erase(pair.first);
        node_values.erase(node_name);
}
...
void HashGraph<T>::load (std::ifstream& in_file, std::string separator) {
        ...
        while (in_file) {
            if (!getline(in_file, line))
                break;
            std::vector<std::string> string = split(line, separator);
            if (string.size() == 1)
                add_node(string[0]);
            else {
                std::istringstream iss(string[2]);
                T val;
                iss >> val;
                add_edge(string[0], string[1], val);
            }
        }
        in_file.close();
}
...
HashGraph<T>& HashGraph<T>::operator = (const HashGraph<T>& rhs){
        ...
        for (auto node: rhs.node_values) {
            node_values[node.first] = rhs.node_values[node.first];
            node_values[node.first].from_graph = this;
        }
        return *this;
}
