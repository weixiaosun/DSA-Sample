...
template<class T, int (*thash) (const T& a)> class HashEquivalence {
  ...
  private:
    ...
    long total_count = 0;
    int maximum_size_of_compress_set = 0;
    int total_size = 0;


    ics::HashMap<T,int> histogram;
...
T HashEquivalence<T,thash>::compress_to_root (T a) {
  ...
  if(maximum_size_of_compress_set < total_count){
      maximum_size_of_compress_set = total_count;
  }



  histogram[compress_set.size()]  += 1;
  total_size += compress_set.size();
  total_count++;
  compress_set.clear();
  return to_root;
}
...
std::string HashEquivalence<T,thash>::equivalence_info () const {
  ...
  double average_compression_set_size;
  ...
  answer << "Maximum size of compress set is " << maximum_size_of_compress_set << std::endl;

  answer << std::endl << " Print histogram of all sizes" << histogram.str() << std::endl;

  answer << std::endl << "Total number of times compression to root was called " << total_count << std::endl;


  average_compression_set_size = (double)total_size/(double)total_count;

  answer << std::endl << " Average number of compression calls " << average_compression_set_size << std:: endl;
