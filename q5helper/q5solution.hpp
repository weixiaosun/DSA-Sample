...
T maximum(const NTN<T>& ntn) {
    T current = ntn.value;
    for (NTN<T> n: ntn.children) {
        current = current > n.value ? current : maximum(n);
    }
    return current;
}
...
bool is_a_word (const DTN& root, std::string remaining_letters) {
  ...
  else if (!root.children.has_key(remaining_letters[0]))
...
std::string longest_word (const DTN& dtn) {
    std::string longest = "";
    if (dtn.children.size() == 0) {
        longest = dtn.word_to_here;
    }
    for (auto c: dtn.children) {
        std::string longest_w = longest_word(c.second);
        longest = longest.size() > longest_w.size() ? longest : longest_w;
    }
    return longest;
}
