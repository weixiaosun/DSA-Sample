...
typedef ics::HashSet<std::string>           FollowSet;
...
//We predict that array implementations are faster because for smaller
//sizes of N the Hash Type data structures are slower because you have to allocate
//multiple bins while you have only one array to go over for the array implementations
//But at an asymptotic level the Hash structures will be faster because the due to the
// threshold factor you won't have to iterate through a lot of bins to read the entire
//corpus.